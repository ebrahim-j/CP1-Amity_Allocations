...
from dbModels import RoomModel, PersonModel, Base
...
class Amity(object):
    ...
    def reallocate_person(self, person_name, new_room_name):
        ...
        for individual in employees:
            if individual.the_name == person_name:
                ...
                if new_room_name.lower() in [room.room_name.lower() for room in self.offices]:
                    for room in self.offices:
                    ...
                    for room in self.l_spaces:
                            if room.room_name == new_room_name and room.room_has_space():
                                ...
                                return "Sorry, room is full"
                    if individual.accommodated != None:
                        for room in self.l_spaces:
                            if individual.the_name in room.current_occupants:
                                room.current_occupants.remove(
                                    individual.the_name)
    ...
    def print_allocations(self, filename=None):
        ...
        if filename is None:
            ...
            return "Data has been successfully saved to {}.txt".format(filename)
...
print(amity.reallocate_person("Yid khb", "Homabay"))
